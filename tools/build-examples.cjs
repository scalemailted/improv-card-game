...
const manifest = {
  schema: 2, version: require('../package.json').version,
  datasetId: 'acted-scenes-' + require('../package.json').version + '-' + sha(JSON.stringify([sourceHashes, allCards.map(fingerprint)])).slice(0, 12),
  entryStatus: 'mixed-review-status',
  reviewScope: {singles: JSON.stringify(require('./local-audit/progress.cjs').progress(root)), bespokePairs: '50 authored scenes; completed coach-1.0 pair reviews counted separately in the checkpoint audit', compiledPairs: 'format-checked drafts, not individually reviewed'},
  formats: ['ABABA'],
...
manifest.assetVersion = manifest.version + '-' + sha(JSON.stringify(manifest)).slice(0,12);
...
// Every changed dataset has fresh app-shell and worker/manifest addresses.
for(const file of ['index.html','app.js','examples/library-client.js','examples/library-worker.js','sw.js']){
 const target=path.join(root,file);if(!fs.existsSync(target))continue;let text=fs.readFileSync(target,'utf8');
 text=text.replace(/\?v=[a-zA-Z0-9.+_-]+/g,'?v='+manifest.assetVersion);
 if(file==='sw.js')text=text.replace(/const CACHE_NAME = "[^"]+";/,'const CACHE_NAME = '+JSON.stringify('imprompt-v'+manifest.assetVersion)+';');
