...
assert.equal(cards.cardPacks.length, 7);
...
assert.equal(audit.summary.activePacks, 7);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 6);
assert.equal(audit.summary.stances, 168);
assert.equal(audit.summary.drives, 168);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, Relationship Knots, Emotional Pressure, Secrets & Schemes, and Absurd Commitment integration passed");
