...
let singles=0,pairs=0,records=0,maxWords=0;const ids=new Set(),formats={ABABA:0},provenance={};
...
for(const [key,f] of Object.entries(m.files)){
 ...
 for(const [id,rs]of Object.entries(doc.records)){
  ...
  assert.ok(m.fingerprints[id]);assert.equal(rs.length,2);assert.ok(rs.every(r=>r.format==='ABABA'));
  assert.equal(new Set(rs.map(r=>JSON.stringify(r.beats))).size,rs.length);
  for(const r of rs){
   records++;assert.ok(!ids.has(r.id));ids.add(r.id);
   assert.equal(r.format,r.beats.map(b=>b.speaker).join(''));assert.equal(r.format,'ABABA');formats[r.format]++;
   if(key==='singles')assert.equal(r.editorialStatus,'internal-editorial-pass');
   const text=r.beats.map(b=>{assert.ok(typeof b.text==='string'&&b.text.length>2);assert.ok(!Object.hasOwn(b,'action'));return b.text;}).join(' ');
   assert.doesNotMatch(text,/[<>{}]|your stance|your drive|your cards|one concrete choice|then heighten/i);
   const words=text.trim().split(/\s+/).length;maxWords=Math.max(words,maxWords);assert.ok(words<=(key==='singles'?75:125));
