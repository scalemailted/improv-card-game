...
assert.equal(cards.cardPacks.length, 5);
...
assert.equal(audit.summary.activePacks, 5);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 4);
assert.equal(audit.summary.stances, 120);
assert.equal(audit.summary.drives, 120);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, Relationship Knots, and Emotional Pressure integration passed");
