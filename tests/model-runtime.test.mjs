  ...
  class Wllama {
    ...
    async createChatCompletion(options) {
      ...
      return (async function* () {
        ...
        yield { choices: [{ delta: { content: "A mock " } }] };
...
test("Adapter uses a bounded CPU context and the v3 completion contract", async () => {
  ...
  assert.deepEqual(result, { text: "A mock example.", finishReason: "stop", chunks: 2 });
  const options = engine.calls.find(c => c[0] === "generate")[1];
  assert.equal(options.abortSignal, signal); assert.equal(options.stream, true); assert.equal(options.max_tokens, 176);
...
test("Streaming reports counts, not unchecked generated prose, and preserves finish reason", async () => {
  const engine = fakeEngine(), progress = [];
  const runtime = createRuntime({ store: { get: async () => new Blob(["GGUF"]) }, importRuntime: async () => engine });
  await runtime.load({}, { allowDownload: true });
  const result = await runtime.generate([{role:"user",content:"test"}], { kind:"single", onProgress: p=>progress.push(p) });
  assert.equal(result.chunks, 2); assert.equal(result.finishReason, "stop");
  assert.equal(progress.at(-1).characters, result.text.length);
  assert.equal(progress.some(p=>Object.hasOwn(p,"text")), false);
  assert.equal(engine.calls.find(c=>c[0]==="generate")[1].max_tokens, 144);
});
test("The adapter makes wrong API contracts and missing completion terminators explicit", async () => {
  for (const response of [
    async () => ({choices:[{message:{content:"Old non-stream contract"}}]}),
    async () => (async function*(){yield {choices:[{delta:{content:"Incomplete stream."}}]};})()
  ]) {
    const engine=fakeEngine(); engine.Wllama.prototype.createChatCompletion=response;
    const runtime=createRuntime({store:{get:async()=>new Blob(["GGUF"])},importRuntime:async()=>engine});
    await runtime.load({}, {allowDownload:true});
    await assert.rejects(runtime.generate([], {kind:"single"}), /expected Wllama|without a finish reason/);
  }
});
test("The adapter retains runtime error/warning information for diagnostics", async () => {
  const engine=fakeEngine();
  const runtime=createRuntime({store:{get:async()=>new Blob(["GGUF"])},importRuntime:async()=>engine});
  await runtime.load({}, {allowDownload:true});
  const options=engine.calls.find(c=>c[0]==="construct")[2];
  for(let i=0;i<15;i++) options.logger.error("failure fixture " + i);
  assert.equal(runtime.describe().logTail.length,10);
  assert.match(runtime.describe().logTail.at(-1).message,/14/);
});
test("A pre-aborted generation never calls into the model", async () => {
  const engine=fakeEngine();
  const runtime=createRuntime({store:{get:async()=>new Blob(["GGUF"])},importRuntime:async()=>engine});
  await runtime.load({}, {allowDownload:true});
  const control=new AbortController();control.abort();
  await assert.rejects(runtime.generate([], {signal:control.signal}), {name:"AbortError"});
  assert.equal(engine.calls.some(c=>c[0]==="generate"),false);
});
