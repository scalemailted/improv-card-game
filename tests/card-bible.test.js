...
assert.equal(cards.libraryPlanVersion, "1.2.0");
assert.equal(cards.stances.length, 72);
assert.equal(cards.drives.length, 72);
assert.equal(cards.cardPacks.length, 3);
...
assert.equal(audit.summary.activePacks, 3);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 2);
assert.equal(audit.summary.stances, 72);
assert.equal(audit.summary.drives, 72);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, and Power Games integration passed");
