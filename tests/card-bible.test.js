...
assert.equal(cards.cardPacks.length, 2);
...
assert.equal(audit.summary.activePacks, 2);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 1);
assert.equal(audit.summary.stances, 48);
assert.equal(audit.summary.drives, 48);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations and Everyday Friction integration passed");
