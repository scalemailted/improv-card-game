...
assert.equal(cards.cardPacks.length, 8);
...
assert.equal(audit.summary.activePacks, 8);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 7);
assert.equal(audit.summary.stances, 192);
assert.equal(audit.summary.drives, 192);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, Relationship Knots, Emotional Pressure, Secrets & Schemes, Absurd Commitment, and Rules, Rituals & Institutions integration passed");
