...
def brief_choice_count(key):
    lengths=[sum(len(b['text'].split()) for b in e['beats']) for e in DATA[key]]
        ...
        for width in WIDTHS:
            ...
            for policy in ['off','after-attempt','nudges']:
                ...
                if policy=='off':expect(page.locator(nudge(KEYS[0]))).to_be_hidden()
                    ...
                    page.click(nudge(KEYS[0]));selected=verify_dialog(page,KEYS[0],width,another=policy!='nudges' or brief_choice_count(KEYS[0])>1)
                    if policy=='nudges':
                    ...
                    if policy=='nudges' and brief_choice_count(KEYS[0])>1:
                        page.click('#anotherHintAngleButton')
                        alternate=verify_dialog(page,KEYS[0],width)
                        assert alternate!=selected
                        assert count(next(e for e in DATA[KEYS[0]] if e['id']==alternate))==min(map(count,DATA[KEYS[0]]))
                    page.click('#doneHintButton')
