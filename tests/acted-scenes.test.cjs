...
test('960 exact-card examples are actual A-B-A-B-A exchanges',()=>{
 assert.equal(Object.keys(src.records).length,480);
 for(const record of Object.values(src.records))for(const e of record.examples){assert.equal(e.beats.map(b=>b.speaker).join(''),'ABABA');assert.ok(e.beats.every(b=>b.text));assert.notEqual(e.beats[0].text,e.beats[2].text);}
});
...
test('brief policy chooses complete shorter five-turn scenes and never launches inference',async()=>{
 ...
 const length=r=>r.beats.reduce((n,b)=>n+b.text.split(/\s+/).length,0);const min=Math.min(...records.map(length));
 for(let i=0;i<4;i++){const result=await lib.next(req);assert.equal(result.example.format,'ABABA');assert.equal(length(result.example),min);assert.equal(result.count,records.filter(r=>length(r)===min).length);}
