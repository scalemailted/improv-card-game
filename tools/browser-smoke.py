...
HTML = HTML.replace('./assets/improv-card-game-qr.png?v=0.22.0', 'data:image/png;base64,' + base64.b64encode((ROOT / 'assets/improv-card-game-qr.png').read_bytes()).decode())
SINGLE = 'Before agreeing to anything, quietly count what the proposal will cost and ask who is paying.'
PAIR = 'Interrupt your own confession to ask for an itemized estimate, then inspect every expense as though the budget is the urgent problem.'
OTHER = 'Start naming the resources you will need whenever you nearly admit the truth, and keep finding one more missing item.'
...
def fixture(browser, policy='full', width=390):
    ...
    for name in SCRIPTS:
        ...
        if name == 'app.js':
            ...
            target = 'runtimeFactory: async () => (await import("./hints/wllama-adapter.mjs?v=0.22.0")).createRuntime(),'
...
with sync_playwright() as pw:
    ...
    page.locator('#combinationHintButton').click()
    ...
    page.locator('#anotherHintAngleButton').click()
    expect(page.locator('#hintFeedback')).to_contain_text('did not pass')
    ...
    print('PASS: opt-in load; concrete fallbacks; single/pair privacy; explicit generation; invalid/canceled output; deck integrity; Veto; Scene Log.')
    page.close()
