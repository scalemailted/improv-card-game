...
function setup() {
  ...
  const caches = { open: async () => cache, match: cache.match, keys: async () => ["imprompt-v0.21.2", "imprompt-v0.22.1", "imprompt-llm-runtime-3.6.1", "unrelated-cache"], delete: async key => deleted.push(key) };
...
test("Offline app shell contains every HTML script and local adapter dependency, but no external model/runtime", async () => {
  ...
  for (const module of ["hints/model-store.mjs", "hints/runtime-config.mjs", "hints/wllama-adapter.mjs?v=0.22.1"]) {
...
test("HTML identifiers and the installed package version are internally consistent", () => {
  ...
  assert.equal(require("../package.json").version, "0.22.1"); assert.doesNotMatch(html, /\?v=0\.21\./);
});
