...
assert.equal(cards.cardPacks.length, 4);
...
assert.equal(audit.summary.activePacks, 4);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 3);
assert.equal(audit.summary.stances, 96);
assert.equal(audit.summary.drives, 96);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, and Relationship Knots integration passed");
