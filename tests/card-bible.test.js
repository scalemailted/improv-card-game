...
assert.equal(cards.cardPacks.length, 6);
...
assert.equal(audit.summary.activePacks, 6);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 5);
assert.equal(audit.summary.stances, 144);
assert.equal(audit.summary.drives, 144);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, Relationship Knots, Emotional Pressure, and Secrets & Schemes integration passed");
