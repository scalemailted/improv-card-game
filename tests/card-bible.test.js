...
const advanced = require("../cards/advanced-scene-engines.js");
...
assert.equal(advanced.stances.length, 24);
assert.equal(advanced.drives.length, 24);
assert.equal(advanced.status, "playtest");
assert.equal(cards.libraryPlanVersion, "1.10.0");
assert.equal(cards.stances.length, 240);
assert.equal(cards.drives.length, 240);
assert.equal(cards.cardPacks.length, 10);
...
assert.equal(audit.summary.activePacks, 10);
assert.equal(audit.summary.publishedPacks, 1);
assert.equal(audit.summary.playtestPacks, 9);
assert.equal(audit.summary.stances, 240);
assert.equal(audit.summary.drives, 240);

console.log("✓ Imprompt Card Bible taxonomy, ten-pack plan, and Core Foundations, Everyday Friction, Power Games, Relationship Knots, Emotional Pressure, Secrets & Schemes, Absurd Commitment, Rules, Rituals & Institutions, and Competition & Consequences, and Advanced Scene Engines integration passed");
