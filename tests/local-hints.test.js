...
const validText = '[I set my cup at the head of the table.] “You may bring the proposal to me now.”';
...
test("Validation rejects unfinished, long, repeated, scripting and generic answers", () => {
  ...
  assert.equal(core.validateOutput(validText, request, "length").ok, true); // A complete, closed moment at the exact token limit is usable.
  assert.equal(core.validateOutput(validText.slice(0,-1), request, "length").reason, "unfinished");
